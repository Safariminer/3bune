...
function TreatString($message): string{
    ...
    if(stripos($message, "tag:") === 0){
        ...
        $tag = $finalmessage;
        $finalmessage = "tag:<span class=\"id" . substr($tag, 4, 14);
...
foreach($messages as $message){
    echo "<time mouseover=\"TimeLookup()\" onclick=\"GetTime()\" class=\"id" . $message->getAttribute('time') . "\">" . $message->getAttribute('time') . "</time> | " . htmlspecialchars($message->childNodes->item(0)->textContent) . " : " . TreatString($message->childNodes->item(1)->textContent) . "<br/>";
}
